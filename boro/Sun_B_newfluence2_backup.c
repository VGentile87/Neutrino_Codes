...
void Sun_B_newfluence2() {
  ...
  Double_t euler_phi = TMath::ATan(TMath::Cos(bx)*TMath::Sin(lz-lx)/TMath::Cos(by)*TMath::Sin(lz-ly));
  //Double_t euler_phi = TMath::ATan(TMath::Cos(by)*TMath::Sin(lz-ly)/TMath::Cos(bx)*TMath::Sin(lz-lx));
  Double_t euler_the = (TMath::Pi()-29.811)*TMath::Pi()/180.;
  ...
  */
  ...
  for(int i=0; i<n_el;i++){
    ...
    // euler_xsi = pthe;
    /*TVector3 v1(1,1,0); // v3 = (1,2,3)
    cout <<"r1 "<< v1(0) << " " << v1(1) << " " << v1(2) << endl;
    TRotation rot;
    rot.SetXEulerAngles(pi/2.,pi/2.,pi/2.);
    v1.Transform(rot);
    cout <<"r2 "<< v1(0) << " " << v1(1) << " " << v1(2) << endl;*/
	...
	///////////////////////////////////////////////////
	t = r->Uniform(-pi,pi);
	px = TMath::Cos(bx)*TMath::Sin(t-lx+lz);  // centro galattico
	py = TMath::Cos(by)*TMath::Sin(t-ly+lz);  // cigno
	pz = TMath::Cos(bz)*TMath::Sin(t-lz+lz);  // normale a centro galattico
	pphi = TMath::ATan2(px,py);
	pthe = TMath::ATan2(pz,TMath::Sqrt(px*px + py*py));
	//pthe = TMath::ATan(TMath::Sqrt(px*px + py*py)/pz);
	//if(pthe<0)pthe = pthe + 2*pi;
	//if(pphi<0)pphi = pphi + 2*pi;

	euler_xsi = pphi;
	TVector3 v1(TMath::Cos(phi_sc)*TMath::Cos(theta_sc),TMath::Sin(phi_sc)*TMath::Cos(theta_sc),TMath::Sin(theta_sc)); // v3 = (1,2,3)	
	cout <<"r1 "<< v1(0) << " " << v1(1) << " " << v1(2) << endl;
	TRotation rot;
	TRotation rot_inv;
	...
	/////////////////////////////////////////////////////
	theta_sum = TMath::ATan2(v1(2),TMath::Sqrt(v1(0)*v1(0)+v1(1)*v1(1)));
	phi_sum = TMath::ATan2(v1(1),v1(0));
	/*if(theta_sum>0) theta_sum = pi/2. - TMath::ATan2(v1(2),TMath::Sqrt(v1(0)*v1(0)+v1(1)*v1(1)));
	if(phi_sum>0) phi_sum = pi/2. -  TMath::ATan2(v1(1),v1(0));
	if(theta_sum<0) theta_sum = -pi/2. - TMath::ATan2(v1(2),TMath::Sqrt(v1(0)*v1(0)+v1(1)*v1(1)));
	if(phi_sum<0) phi_sum = - pi/2. -  TMath::ATan2(v1(1),v1(0));*/
	...
	h0->Fill(cos_theta_sum,atomic_fract_el[i]*A_el[i]*A_el[i]);
	h3->Fill(theta_sc,atomic_fract_el[i]*A_el[i]*A_el[i]);
	h33->Fill(phi_sc,atomic_fract_el[i]*A_el[i]*A_el[i]);
