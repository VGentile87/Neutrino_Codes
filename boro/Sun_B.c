...
void Sun_B() {
  ...
  TF1 *fthe = new TF1("fthe","atan2(sqrt(fx*fx+fy*fy),fz)",0,2*TMath::Pi());
  TH1F *hr = new TH1F("hr","hr",100,0,2*TMath::Pi());
  TH1F *hphi = new TH1F("hphi","hphi",100,-TMath::Pi(),TMath::Pi());
  TH1F *hthe = new TH1F("hthe","hthe",100,-pi,pi);
  ...
  for(int i=0;i<100000;i++){
    ...
    pz = TMath::Cos(bx)*TMath::Sin(t-lz+ly);
    pphi = TMath::ATan2(px,py);
    pthe = TMath::ATan2(TMath::Sqrt(px*px + py*py),pz);
    if(pthe > TMath::Pi()/2.) {
      cout << pthe << endl;
      pthe = TMath::Pi() - pthe;
      cout << pthe << endl;
  ...
  for(int i=0; i<n_el;i++){
	...
	if(L_true > 50 && L_true<1000 /*&& E_rec>0.002*/){
	  num_ev_bkg = functions->num_eventi_bkg();
	  sum_ev_bkg = sum_ev_bkg + num_ev_bkg;
  ...
  TCanvas *cc2 = new TCanvas("cc2","cc2",1200,400);
  cc2->Divide(3,1);
  cc2->cd(1);
  hr->Draw("");
  cc2->cd(2);
