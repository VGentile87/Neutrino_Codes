...
class Functions
{
 public: 
  ...
  inline double cross_section(){
    ...
    crsect = ((GF*GF*iQw*Fq*Fq*iM_el*1000)/(4*pi))*(1-iM_el*E_rec/(2*E_nu*E_nu))*TMath::Power(iMeV_in_fm,2)*TMath::Power(10,-26); //MeV
    return crsect;
  }
  ...
  inline double num_eventi_bkg(){
    ...
    dN_vs_dE_bkg = int_const_bkg*crsect*iflux_boron_vs_E/1000.;
    //cout << int_const_bkg << " " << dN_vs_dE_bkg << endl;
    return dN_vs_dE_bkg;
  }
