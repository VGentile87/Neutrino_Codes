...
void nu_floor() {
  ...
  for(int k=0;k<100000;k++){
	...
	//iflux_boron_vs_E=(B_nu_rate[j]+tmp_B_rate)*(1./50.)/2.;
	...
	if(E_nu >(TMath::Sqrt(iM_el*E_rec/2.))){
	  //if(crsect<0)cout << E_nu << " " << TMath::Sqrt(iM_el*E_rec/2.) << " " << crsect << endl;
	...
	tmp_B_rate=B_nu_rate[j];
      ...
      //exp1 = (R0/(E0*r))*((TMath::Power(pi,0.5)/4.)*(v0/vE)*(TMath::Erf((vmin+vE)/v0)-TMath::Erf((vmin-vE)/v0))-TMath::Exp(-(vesc*vesc)/(v0*v0)));
