...
void Atm_nu() {
  ...
  log_file << "Atmospheric induced recoil rate";
  log_file << endl;
  ...
  for(int i=0; i<n_el;i++){    
    ...
    for(int j=0; j<tot_line;j++){
      ...
      E_nu=nu_en[j];
      sum_crsect=0;
      //iflux_atm_vs_E=nu_rate[j]*(E_nu-tmp_E_nu);
      iflux_atm_vs_E=(E_nu-tmp_E_nu)*(nu_rate[j]+tmp_nu_rate)/2.;
      if(j>0)sum_flux += iflux_atm_vs_E;
      //cout << j << " " << E_nu << " " << iflux_atm_vs_E  <<  " " << sum_flux << " " << nu_rate[j] << " " << tmp_nu_rate << endl;
      //cout << (E_nu-tmp_E_nu) << " " << (nu_rate[j]+tmp_nu_rate) << endl;
