...
TSpline3 *RangeVersusEnergy(Int_t charge){
  ...
  while(getline(inputFile, line)) {
    ...
    if(num_line<=80){
      ...
      if(ene_unit[0]=='k')ene=ene/1000.; //GeV
      if(ene_unit[0]=='M')ene=ene;       //GeV
      if(len_unit[0]=='A')len=len/10000.; // nm
      if(len_unit[0]=='u')len=len; // nm
...
TSpline3 *EnergyVersusRange(Int_t charge){
  ...
  while(getline(inputFile, line)) {
    ...
    if(num_line<=80){
      ...
      if(ene_unit[0]=='k')ene=ene/1000.; //GeV
      if(ene_unit[0]=='M')ene*=1.;  //GeV
      if(len_unit[0]=='A')len/=10000.; // nm
      if(len_unit[0]=='u')len*=1.; // nm
      //std::cout<<"point: "<<ene<< " keV " << len<<' '<< " nm " <<std::endl;
      //printf("i %d -> ENE %4.4f LEN %4.4f\n",Narray,ene,len);
...
void DrawCorrelation(){
  ...
  TSpline3 *Line_C  = RangeVersusEnergy(6);
  TSpline3 *Line_N  = RangeVersusEnergy(7);
  TSpline3 *Line_O  = RangeVersusEnergy(8);
  TSpline3 *Line_S  = RangeVersusEnergy(16);
  TSpline3 *Line_Br = RangeVersusEnergy(35);
  TSpline3 *Line_Ag = RangeVersusEnergy(47);
  TSpline3 *Line_I =  RangeVersusEnergy(53);
  
  vC2 = new TCanvas("vC2","square",200,10,700,700);
  vC2->cd();

  TH1F *vFrame = gPad->DrawFrame(0,0,1,1);
  vFrame->Draw();
  vFrame->GetXaxis()->SetTitleOffset(1.3);
  vFrame->GetYaxis()->SetTitleOffset(1.6);
  vFrame->SetYTitle("Range (nm)");
  vFrame->SetXTitle("Energy (keV)");
  
  //Line_H->SetLineColor(kCyan);
  //Line_H->SetLineWidth(2.0);
  Line_C->SetLineColor(kRed+1);
  Line_C->SetLineWidth(3);
  Line_N->SetLineColor(kOrange-3);
  Line_N->SetLineWidth(3);
  Line_O->SetLineColor(kYellow);
  Line_O->SetLineWidth(3.0);
  Line_S->SetLineColor(kGreen);
  Line_S->SetLineWidth(3.0);
  Line_Br->SetLineColor(kCyan);
  Line_Br->SetLineWidth(3.0);
  Line_Ag->SetLineColor(kBlue+1);
  Line_Ag->SetLineWidth(3.0);
  Line_I->SetLineColor(kViolet);
  Line_I->SetLineWidth(3.0);
