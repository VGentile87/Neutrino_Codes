...
void SignificanceEvaluation_100nm_v4()
{
  //ofstream log_file("log_1ton_1y.txt",ios::app);
  const int Npoints=7;
  const double mass_exposure=3; // ton
  double nsig = 0;
  double nsig_1y=2.18;
  double nbkg = 0;
  double nbkg_1y=0.53;
	       ...
	       double nbkg, double sigmab)   // number of background events
{
  ...
  RooRealVar len("len","len",0.1,1);
  RooRealVar phi("phi","phi",-pi,pi);
  the.setBins(10);
  len.setBins(10);
  phi.setBins(10);
  /*
  TFile *file = new TFile("allhisto_v9_100nm.root");
...
*/
  TFile *file = new TFile("allhisto_v11_100nm.root");
