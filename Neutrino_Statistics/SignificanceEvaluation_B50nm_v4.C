...
void SignificanceEvaluation_B50nm_v4()
{
  ...
  for(int ip=0; ip<Npoints; ip++){
    ...
    nsig = 1.32*time_exposure[ip]*mass_exposure;
    nbkg = 0.33*time_exposure[ip]*mass_exposure;
	       ...
	       double nbkg, double sigmab)   // number of background events
{
  ...
  */
  ...
  w.factory("PROD:sig_pdf(bthepdf,bphipdf,blenpdf)");
  w.factory("PROD:bkg_pdf(nthepdf,nphipdf,nlenpdf)");
